...
<style>
/* باکس صورت ها */
.dash-soraat-bar { direction: rtl; box-shadow: 0 4px 24px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.04); border-radius: 1rem; background: #fff; overflow: hidden; }
...
.dash-soraat-bar .dash-soraat-scroll { display: flex; flex-direction: row; flex-wrap: nowrap; overflow-x: auto; overflow-y: visible; gap: 0.75rem; padding: 0.6rem; align-items: stretch; scrollbar-width: thin; scrollbar-color: #94a3b8 #f1f5f9; }
...
.dash-soraat-bar .dash-soraat-card { flex: 0 0 auto; min-width: 280px; max-width: 320px; background: #fff; border-radius: 0.875rem; padding: 0; margin: 0; box-shadow: 0 2px 12px rgba(0,0,0,0.06); border: 1px solid #e5e7eb; font-size: 12px; overflow: hidden; }
                ...
                <form wire:submit.prevent="editPriceOnSoraats(Object.fromEntries(new FormData($event.target)), '{{ $groupId }}')"
                    ...
                    @foreach($messages as $message)
                        <div class="soraat-item-{{ $groupId }} dash-soraat-row {{ Str::endsWith(trim($message->code), ': -') ? 'italic' : '' }}">
                            ...
                            @else
                                <input type="text" name="price.{{ $message->id }}">
                            @endif
                        </div>
                    @endforeach
                    <div class="dash-soraat-footer">
                        ...
                        <div class="dash-copy-btns">
                            ...
                            <button type="button" class="blue" onclick="copySoraatGroup('{{ $groupId }}', this, 'codes')" title="کپی کدها">
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                            </button>
                        </div>
                    </div>
                </form>
